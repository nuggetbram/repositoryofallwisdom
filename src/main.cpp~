...
unsigned int nStakeMinAge = 24 * 60 * 60;	// time at which weight begins to build
unsigned int nStakeMinAgeV2 = 24 * 60 * 60;	// functionally the minimum age is 8.8 days
unsigned int nStakeMaxAge = 60 * 60 * 24 * 30;	// stake age of full weight: -1
unsigned int nStakeTargetSpacing = 60 *10;			// 90 sec block spacing

int64 nChainStartTime = 1424163276;
...
// simple algorithm, not depend qon the diff
...
bool CBlock::AcceptBlock()
{
	...
	/*	GreenCoinX is not using a checkpoint server
...
bool ProcessBlock(CNode* pfrom, CBlock* pblock)
{
	...
	/* GreenCoinX is not using checkpoint server
	...
	// presstab GreenCoinX: enable of disable staking based on block difficulty
...
bool CheckDiskSpace(uint64 nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "GreenCoinX", CClientUIInterface::OK | CClientUIInterface::ICON_ETRKLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
unsigned char pchMessageStart[4] = { 0xf4, 0xe0, 0xd0, 0x31 };
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
		...
		/* GreenCoinX does not use checkpoint server
...
CBlock* CreateNewBlock(CWallet* pwallet, bool fProofOfStake)
{
    ...
    if (fProofOfStake && !pwalletMain->fDisableStake)  // attempt to find a coinstake && make sure settings allow PoS (presstab GreenCoinX)
    {
