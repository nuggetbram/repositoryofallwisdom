            ...
            while (true)
            {
                ...
                if (nChange > 0)
                {
                    ...
                    else
                    {
						...
						// change GreenCoinX to get rid of automatic generation of change address and instead send change back
...
{
    //presstab GreenCoinX: return false if disablestake RPC is enabled
	if(fDisableStake)
		return false;
    ...
    BOOST_FOREACH(PAIRTYPE(const CWalletTx*, unsigned int) pcoin, setCoins)
    {
		...
		//GreenCoinX now iterates each utxo inside of CheckStakeKernelHash()
